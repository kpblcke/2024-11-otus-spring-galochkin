...
import ErrorContainer from "../../components/container/ErrorContainer.jsx";
...
const getBookApiData = async (id, setErrorText) => {
   return await fetch(
        "/api/v1/books/" + id
    ).then((response) => {
        if (response.status === 404) {
            setErrorText("Книга не найдена");
        } else {
            setErrorText("");
        }
        response.json()
...
const putNewBookApiData = async (book, author, genre, setErrors, navigate) => {
    const response = await fetch(
        "/api/v1/books",
        {
            ...
            body: JSON.stringify({title: book, authorId: author, genresIds: genre})
        }
    ).then((response) => response.json());
    setErrors(response);
...
const putEditBookApiData = async (id, book, author, genre, setErrors, navigate) => {
    const response = await fetch(
        "/api/v1/books",
        {
            ...
            body: JSON.stringify({id: id, title: book, authorId: author, genresIds: genre})
        }
    ).then((response) => response.json());
    setErrors(response);
    if (response.hasError === false) {
...
const BookEdit = (props) => {
            ...
            if (id) {
                try {
                    const [bookData, authorsData, genresData] = await Promise.all([
                        getBookApiData(id, setErrorText),
                        getAuthorsApiData(),
                        getGenresApiData()
                    ]);
    ...
    return (
        <React.Fragment>
            ...
            <ErrorContainer error={errorText ? errorText : ""}/>
            <CssBaseline/>
